...
-- officers/admin can upload new photos during submission. The `name ~ ...`
-- check constrains the path to the exact shape the app generates itself
-- (block/school_timestamp.ext, from safeFileSegment() in storage.ts) — so a
-- valid officer session used directly against the Storage API (bypassing the
-- app entirely, e.g. via curl/Postman) can't dump arbitrary junk files or
-- paths into the bucket; it can only upload things shaped like a real
-- inspection photo.
create policy "officer upload photos" on storage.objects
  for insert with check (
    bucket_id = 'inspection-photos'
    and public.is_officer_or_admin()
    and name ~ '^[A-Za-z0-9_]{1,60}/[A-Za-z0-9_]{1,80}\.(jpg|jpeg|png|webp)$'
  );

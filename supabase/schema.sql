...
begin
  ...
  insert into inspections (
    id, created_at, block, school_name, school_category, management_type, meal_served,
    student_count, expected_student_count, attendance_boys, attendance_girls,
    aadhaar_boys, aadhaar_girls, photo_path, latitude, longitude, remarks, issue_category,
    ...
    new_id, created, payload->>'block', v_school_name, payload->>'school_category', payload->>'management_type', payload->>'meal_served',
    coalesce((payload->>'student_count')::int, 0), coalesce((payload->>'expected_student_count')::int, 0),
    (payload->>'attendance_boys')::int, (payload->>'attendance_girls')::int,
    (payload->>'aadhaar_boys')::int, (payload->>'aadhaar_girls')::int,
    payload->>'photo_path', payload->>'latitude', payload->>'longitude', payload->>'remarks', payload->>'issue_category',
  ...
  select count(*) filter (where meal_served = 'yes'), count(*)
    into served_count, total_count
    from inspections where inspections.school_name ilike v_school_name;

  compliance := round((coalesce(served_count, 0)::numeric / greatest(coalesce(total_count, 0), 1)) * 100);
  ...
  insert into schools (id, name, block, category, enrolled_students, last_inspected, compliance_rate)
  values (new_school_id, v_school_name, payload->>'block', primary_category, coalesce((payload->>'expected_student_count')::int, 0), created, compliance)
...
-- (compliance_rate, headmaster_contact, last_inspected) — just enough to
-- power a dropdown.

...
export function Dashboard({ inspections, onDataChanged }: DashboardProps) {
  ...
  // Full report + photo, opened from Recent Submissions (richer than the plain photo preview above)
  const [selectedSubmission, setSelectedSubmission] = useState<InspectionRecord | null>(null);
  ...
  // Only schools that actually have a problem should ever appear on these two
  // "worst performer" leaderboards. Without a bar to clear, a school with a
  // perfect score could still show up simply for being the lowest of however
  // many schools exist (e.g. the only school in the data) -- misleading, since
  // the card is styled as a warning/watchlist, not a neutral ranking.
  const AADHAAR_COVERAGE_THRESHOLD_PCT = 90; // below this = flagged
  const { aadhaarRanked, reportingRanked } = useMemo(() => {
    ...
    schoolLatest.forEach(({ record, block, recordCount }, schoolName) => {
      ...
      if (attendanceTotal > 0) {
        ...
        if (pct < AADHAAR_COVERAGE_THRESHOLD_PCT) {
      ...
      if (answered.length > 0) {
        ...
        // Flagged only if at least one of the 3 required reports is a
        // confirmed "no" -- fully compliant schools (100%, or not yet
        // answered) never appear here.
        if (yes < answered.length) {
  ...
  const RANK_CONFIG: Record<RankKey, { title: string; subtitle: string; metricColumnLabel: string; rows: SchoolRankRow[]; filename: string }> = {
    aadhaar: { title: 'Lowest Aadhaar Coverage', subtitle: `${PERIOD_LABELS[overviewPeriod]} \u2022 schools below ${AADHAAR_COVERAGE_THRESHOLD_PCT}% (Aadhaar-linked \u00f7 attendance) at their latest inspection`, metricColumnLabel: 'Aadhaar Coverage', rows: aadhaarRanked, filename: 'Lowest_Aadhaar_Coverage' },
    reporting: { title: 'Lowest Reporting Compliance', subtitle: `${PERIOD_LABELS[overviewPeriod]} \u2022 schools missing at least one of SDSEO / MeghSIMS / foodgrain reporting`, metricColumnLabel: 'Reporting Compliance', rows: reportingRanked, filename: 'Lowest_Reporting_Compliance' },
  };
  ...
  // One row in the submission-detail modal: a label, a yes/no/value, and an
  // optional reason shown only when the answer was "no".
  function DetailRow({ label, value, reason }: { label: string; value?: string; reason?: string }) {
    if (value === undefined || value === null || value === '') return null;
    const isNo = value === 'no';
    return (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, padding: "8px 0", borderTop: `1px solid ${c.line}` }}>
        <span style={{ fontSize: 12, color: c.textSecondary }}>{label}</span>
        <div style={{ textAlign: "right" }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: isNo ? c.terracotta : c.ink, textTransform: "capitalize" }}>{value}</span>
          {isNo && reason && <div style={{ fontSize: 11, color: c.textFaint, marginTop: 2, maxWidth: 220 }}>{reason}</div>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fade-in" style={{ padding: isMobile ? "16px 12px" : "32px 24px", maxWidth: 1200, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
      ...
      <button
        ...
        style={{
          background: c.surface, borderRadius: 14, padding: isMobile ? "10px 14px" : "12px 16px", border: `1px solid ${c.line}`, boxShadow: shadows.sm,
          cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 12, width: "100%",
          marginBottom: isMobile ? 14 : 18
        }}
      >
        <div style={{ width: 32, height: 32, borderRadius: 9, background: c.mint, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <ClipboardCheck size={16} color={c.forest} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: c.ink, letterSpacing: "-0.02em" }}>{schoolsInspectedCount}</div>
          <div style={{ fontSize: 11, color: c.textSecondary, fontWeight: 500 }}>Schools inspected • For: {PERIOD_LABELS[overviewPeriod]} • Tap for details</div>
        </div>
        ...
        {recentSubmissions.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {recentSubmissions.map(insp => (
              <button
                key={insp.id}
                onClick={() => setSelectedSubmission(insp)}
                style={{
      ...
      <div style={{
        ...
        {([
          { key: 'aadhaar' as const, title: 'Lowest Aadhaar Coverage', icon: UserCheck, rows: aadhaarRanked, explainer: `Schools below ${AADHAAR_COVERAGE_THRESHOLD_PCT}% Aadhaar-linked (of students present), at each school's latest inspection.` },
          { key: 'reporting' as const, title: 'Lowest Reporting Compliance', icon: ShieldCheck, rows: reportingRanked, explainer: "Schools missing at least one of the 3 required reports (SDSEO form, MeghSIMS daily, foodgrain) at their latest inspection." },
        ]).map(panel => (
      ...
      <div style={{ background: c.surface, padding: "12px 16px", borderRadius: 14, border: `1px solid ${c.line}`, boxShadow: shadows.sm, marginBottom: isMobile ? 20 : 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: c.ink }}>Inspections by Block</span>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 10, color: c.textFaint }}>For: {PERIOD_LABELS[overviewPeriod]}</span>
            <Building2 size={14} color={c.forest} />
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {blockStats.map((block) => (
            <div key={block.name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: block.total === 0 ? c.textFaint : c.ink, width: isMobile ? 76 : 100, flexShrink: 0 }}>{block.name}</span>
              <div style={{ flex: 1, height: 6, background: c.mint, borderRadius: 3, overflow: "hidden" }}>
                <div style={{
                  width: block.total === 0 ? "0%" : `${Math.round((block.total / maxBlockTotal) * 100)}%`,
                  height: "100%", background: block.total === 0 ? c.line : c.forest, borderRadius: 3,
                  transition: "width 0.6s cubic-bezier(0.16, 1, 0.3, 1)"
                }} />
              </div>
              <span style={{ fontSize: 11, fontWeight: 700, color: block.total === 0 ? c.textFaint : c.forest, width: 16, textAlign: "right" }}>{block.total}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Submission Detail Modal -- opened from Recent Submissions: the full report, plus photo */}
      {selectedSubmission && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ position: "relative", maxWidth: 480, width: "100%", maxHeight: "90vh", overflowY: "auto", background: "#FFFFFF", border: `1px solid ${c.line}`, borderRadius: 16, boxShadow: shadows.md }}>
            ...
            <div style={{ padding: "16px 20px 20px" }}>
              <div style={{ marginBottom: 4 }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: c.ink }}>{selectedSubmission.schoolName}</div>
                <div style={{ fontSize: 12, color: c.textSecondary }}>
                  {selectedSubmission.block} Block • {selectedSubmission.schoolCategory} • {new Date(selectedSubmission.timestamp).toLocaleDateString()} {new Date(selectedSubmission.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </div>
                {selectedSubmission.inspectorName && (
                  <div style={{ fontSize: 12, color: c.textFaint, marginTop: 2 }}>Inspected by {selectedSubmission.inspectorName}</div>
                )}
              </div>

              <DetailRow label="Students Present" value={String(selectedSubmission.studentCount)} />
              <DetailRow label="Attendance (B/G)" value={selectedSubmission.attendanceBoys !== undefined || selectedSubmission.attendanceGirls !== undefined ? `${selectedSubmission.attendanceBoys ?? 0} / ${selectedSubmission.attendanceGirls ?? 0}` : undefined} />
              <DetailRow label="Aadhaar Linked (B/G)" value={selectedSubmission.aadhaarBoys !== undefined || selectedSubmission.aadhaarGirls !== undefined ? `${selectedSubmission.aadhaarBoys ?? 0} / ${selectedSubmission.aadhaarGirls ?? 0}` : undefined} />
              <DetailRow label="Kitchen Shed Functional" value={selectedSubmission.kitchenShed} reason={selectedSubmission.kitchenShedReason} />
              <DetailRow label="Water Supply" value={selectedSubmission.waterSupply} reason={selectedSubmission.waterSupplyReason} />
              <DetailRow label="Foodgrains Delivered" value={selectedSubmission.foodgrainsDelivered} />
              <DetailRow label="Kitchen Garden" value={selectedSubmission.kitchenGarden} reason={selectedSubmission.kitchenGardenReason} />
              <DetailRow label="Meals Served (All 5 Days)" value={selectedSubmission.mealsServedAllFiveDays} reason={selectedSubmission.missedMealDaysReason} />
              <DetailRow label="Submitted to SDSEO" value={selectedSubmission.submittedSDSEO} reason={selectedSubmission.sdseoNonSubmissionReason} />
              <DetailRow label="MeghSIMS Daily" value={selectedSubmission.meghSimsDaily} reason={selectedSubmission.meghSimsNoReason} />

              {selectedSubmission.remarks && (
                <div style={{ marginTop: 12, padding: "10px 12px", background: c.paper, borderRadius: 10, border: `1px solid ${c.line}` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: c.textSecondary, marginBottom: 4 }}>Remarks</div>
                  <div style={{ fontSize: 12, color: c.ink, lineHeight: 1.4 }}>{selectedSubmission.remarks}</div>
                </div>
              )}

              {selectedSubmission.photoUrl && (
                <button
                  onClick={() => downloadInspectionPhoto(selectedSubmission).catch(e => console.error(e))}
                  style={{
                    marginTop: 14, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                    padding: "9px 14px", borderRadius: 10, background: c.forest, border: "none", fontSize: 12, fontWeight: 700, color: "#FFFFFF", cursor: "pointer"
                  }}
                >

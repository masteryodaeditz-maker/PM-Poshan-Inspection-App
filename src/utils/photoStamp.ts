// Burns inspection context — date/time, school, inspector, GPS
...
//
// Deliberately excludes the administrative "block" field: block is a
// manually-picked dropdown value on the form, never validated against the
// actual GPS coordinates, so stamping it onto the photo would make it look
// like verified evidence when it isn't. Block still lives in the form and
// database for record-keeping — it's just not drawn on the image.
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const headerParts = [info.schoolName].filter(Boolean) as string[];

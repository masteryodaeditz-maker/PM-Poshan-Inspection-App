...
export async function exportInspectionsPDF(
  ...
  for (let idx = 0; idx < inspections.length; idx++) {
    ...
    const issueText = insp.issueCategory || 'None';
    ctx.page.drawText('Issue category', { x: infoX, y: rowTop - 26, size: 9, font: regular, color: COLORS.textSecondary });
    ctx.page.drawText(issueText, { x: PAGE.width - MARGIN.right - bold.widthOfTextAtSize(issueText, 9), y: rowTop - 26, size: 9, font: bold, color: issueText === 'None' ? COLORS.ink : COLORS.danger });
    ...
    // ---- Meal and attendance ----
    drawSectionLabel(ctx, 'Meal and attendance');
    drawTwoColumnRow(
      ctx, 'Meal served', fmtYesNo(insp.mealServed),
      'Students served', `${insp.studentCount ?? 0} / ${insp.expectedStudentCount ?? 0}`,
      insp.mealServed === 'no' ? COLORS.danger : COLORS.ink
    );

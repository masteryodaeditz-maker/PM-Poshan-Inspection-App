...
export async function exportInspectionsPDF(
  ...
  for (let idx = 0; idx < inspections.length; idx++) {
    ...
    const photoStatusText = insp.photoUrl ? 'Attached' : 'Not attached';
    ctx.page.drawText('Photo evidence', { x: infoX, y: rowTop - 10, size: 9, font: regular, color: COLORS.textSecondary });
    ctx.page.drawText(photoStatusText, { x: PAGE.width - MARGIN.right - bold.widthOfTextAtSize(photoStatusText, 9), y: rowTop - 10, size: 9, font: bold, color: COLORS.ink });
